...
export async function getViralMetrics(): Promise<ViralMetrics> {
  try {
    ...
    // Use EST timezone for "today" calculations
    // Calculate start of today in EST properly using Intl formatter
    const now = new Date();
    ...
    const estYear = parseInt(estParts.find(p => p.type === 'year')?.value || '2026');
    const estMonth = parseInt(estParts.find(p => p.type === 'month')?.value || '1') - 1;
    const estDay = parseInt(estParts.find(p => p.type === 'day')?.value || '1');
    // Midnight EST = 5am UTC (in winter, EST is UTC-5)
    const todayStartUTC = new Date(Date.UTC(estYear, estMonth, estDay, 5, 0, 0));
...
export async function getShareMetrics(): Promise<ShareMetrics> {
  try {
    ...
    // Calculate start of today in EST properly using UTC methods
    const now = new Date();
    // Get current time in EST by using Intl formatter
    const estFormatter = new Intl.DateTimeFormat('en-US', {
      timeZone: 'America/New_York',
      year: 'numeric', month: '2-digit', day: '2-digit'
    });
    ...
    const estMonth = parseInt(estParts.find(p => p.type === 'month')?.value || '1') - 1;
    const estDay = parseInt(estParts.find(p => p.type === 'day')?.value || '1');
    // Create midnight EST as UTC: midnight EST = 5am UTC (in winter)
    // Use Date.UTC to avoid local timezone issues
    const todayStartUTC = new Date(Date.UTC(estYear, estMonth, estDay, 5, 0, 0)); // 5am UTC = midnight EST
